...
def draw_parallel_lines(n, point, length, window):
    ...
    # -------------------------------------------------------------------------
    for k in range(n):
        spot = rg.Point(point.x, point.y + 30 * k)
        line = rg.Line(spot, rg.Point(spot.x + length, spot.y))
...
def draw_lines(n, point, window):
    ...
    # -------------------------------------------------------------------------
    for k in range(n):
        spot = rg.Point(point.x + 100, point.y - 100 + k*(200/n-1))
        line = rg.Line(point, spot)
        line.attach_to(window)
    window.render()
